        ...
        for ($i=1; $i <= 4 ; $i++) {
            ...
            if ($realisasi[$i] > 0) {
                ...
                if (preg_replace("/[^0-9]/","", request('realisasi_' . $i)) > 0) {
                    ...
                    $hasill[$i] = (hitungNilaiSerapan(date('Y'), cekCurrentTriwulan()['current']->triwulan, Auth::user()->id) / 6) * cekPersenSerapan(date('Y'), 2, cekCurrentTriwulan()['current']->triwulan)->nilai;
                    ...
                    $reportAssesment[$i] = ReportAssessment::updateOrCreate(
                        ...
                        [
                            'hasil' => $hasill[$i],
                            'nilai' => hitungNilaiSerapan(date('Y'), cekCurrentTriwulan()['current']->triwulan, Auth::user()->id)
                        ]
                    );
