            ...
            @foreach($map_sat as $k => $data)
                <tr class="{{($k % 2 == 0) ? 'even' : ''}}">
                    ...
                    <td align="center" class=" ">
                        <form method="post" action="{{url('mapping-satker/delete')}}">
                            {{csrf_field()}}
                            <input type="hidden" name="satId" value="{{$data->id}}">
                            <button type="submit" class="btn btn-danger" data-toggle="tooltip" title="Hapus Mapping">
