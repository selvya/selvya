...
<style>
    ...
    .widget-content.text-right.animation-pullDown strong{
        margin-right:15px
    }
</style>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="row">
                    ...
                    @php
                        $ssks = \App\NilaiAkhir::where('user_id',Auth::user()->id)
                                ->where('tahun',date('Y'))
                                ->orderBy('triwulan')
                                ->get();
                        $x = count($ssks);
                        ...
                        <div class="col-sm-12 col-lg-6">
                            <a href="" class="widget widget-hover-effect1">
                                <div class="widget-simple">
                                    <div
                                        class="pie-chart block-section"
                                        data-bar-color="{{$color}}"
                                        data-percent="{{$nilaiakhir->nilai}}"
                                        data-size="50"
                                    >
                                        <span>
                                            <b>{{$nilaiakhir->nilai}}</b>
                                        </span>
                                    </div>
                ...
                <div class="block">
                    <p>
                        <center>Selamat datang di dashboard monitoring program budaya OJKway. </center>
                        <br> <br>
    ...
    <script type="text/javascript">
        Highcharts.chart('container', {
            ...
            xAxis: {
                categories: [
                    @php 
                        $ssks = \App\Persentase::leftJoin('daftarindikator','persentase.daftarindikator_id','=','daftarindikator.id')
            ...
            series: [{
                ...
                data: [
                    @php 
                        foreach($ssks as $indi){
                            $nilainya = \App\ReportAssessment::where('daftarindikator_id',$indi->daftarindikator_id)
        ...
        Highcharts.chart('containernya', {
            ...
            series: [{
                ...
                data: [
                    @php 
                        $ssks = \App\ReportAssessment::leftJoin('daftarindikator','report_assesment.daftarindikator_id','=','daftarindikator.id')
                            ->where('user_id',Auth::user()->id)
                            ->where('triwulan',cekCurrentTriwulan()['current']->triwulan)
                            ->where('tahun',date('Y'))
                            ->get();
